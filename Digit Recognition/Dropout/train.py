...
def main():
    ...
    parser.add_argument('--DIREC', type=str,
                        default='FCNet_MCDropout_p0.05_10_25_n', help='project name')
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx % 10 == 0:
                print('[Epoch: %d/%d, Batch: %d/%d] loss: %.4f, accu: %.2f' % 
                      (epoch+1, args.max_epoch, batch_idx+1, len(train_loader),
                       loss.item()/len(data), 100*np.sum(index == target.data.cpu().numpy())/len(data)))
